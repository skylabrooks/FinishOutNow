...
import { logDiscoveryResults } from './planoUtils';
...
export async function fetchPlanoPermits(): Promise<Permit[]> {
  ...
  if (arcgisResult.length > 0) {
    Logger.info(`[Plano] ✓ Successfully fetched ${arcgisResult.length} permits via ArcGIS Feature Service`);
    logDiscoveryResults();
    return arcgisResult;
  }
  ...
  if (energovResult.length > 0) {
    Logger.info(`[Plano] ✅ Successfully fetched ${energovResult.length} permits via EnerGov API`);
    logDiscoveryResults();
  ...
  if (socrataResult.length > 0) {
    Logger.info(`[Plano] ✅ Successfully fetched ${socrataResult.length} permits via Open Data Portal`);
    logDiscoveryResults();
    return socrataResult;
  }

  // All public endpoints failed - log results for human review
  Logger.warn('[Plano] ⚠️  All API endpoints failed. Review discovery results below:');
  logDiscoveryResults();
  Logger.warn('[Plano] 📋 ACTION REQUIRED: Human must contact Plano IT for API access.');
  Logger.warn('[Plano] 📄 See: Permit_API_Implementation/plano_discovery_results.md for details');
  
  // Fallback to mock data
  return getMockPlanoPermits();
};

// TODO: Replace all console.log in this file with Logger for consistency
...
async function tryArcGISEndpoint(): Promise<Permit[]> {
  ...
  for (const baseUrl of endpointsToTry) {
    try {
      Logger.info(`[Plano ArcGIS] Testing: ${baseUrl.substring(0, 50)}...`);
      ...
      if (response.ok) {
        ...
        if (data.features && Array.isArray(data.features) && data.features.length > 0) {
          Logger.info(`[Plano ArcGIS] ✅ Found ${data.features.length} features`);
          Logger.info(`[Plano ArcGIS] Sample fields: ${Object.keys(data.features[0].attributes).slice(0, 10).join(', ')}`);
          
          discoveryResults.push({ endpoint: baseUrl, success: true });
          return parseArcGISPermits(data.features);
        } else {
          Logger.info(`[Plano ArcGIS] Response OK but no features found`);
          discoveryResults.push({ endpoint: baseUrl, success: false, errorType: 'empty_response' });
        }
      } else if (response.status === 401 || response.status === 403) {
        Logger.info(`[Plano ArcGIS] 🔒 Authentication required (${response.status})`);
        discoveryResults.push({ endpoint: baseUrl, success: false, errorType: 'auth_required' });
      } else if (response.status === 404) {
        Logger.info(`[Plano ArcGIS] ❌ Not found (404)`);
        discoveryResults.push({ endpoint: baseUrl, success: false, errorType: 'not_found' });
      } else {
        Logger.info(`[Plano ArcGIS] ⚠️  HTTP ${response.status}`);
      ...
      Logger.warn(`[Plano ArcGIS] ❌ ${errorType}: ${error.message}`);
...
async function tryEnergovEndpoint(): Promise<Permit[]> {
  ...
  for (const baseUrl of endpointsToTry) {
    try {
      Logger.info(`[Plano EnerGov] Testing: ${baseUrl}`);
      ...
      if (response.ok) {
        ...
        if (Array.isArray(data) && data.length > 0) {
          Logger.info(`[Plano EnerGov] ✅ Found ${data.length} permits (no auth required!)`);
          discoveryResults.push({ endpoint: baseUrl, success: true });
          return parseEnergovPermits(data);
        } else {
          Logger.info(`[Plano EnerGov] Response OK but empty data`);
          discoveryResults.push({ endpoint: baseUrl, success: false, errorType: 'empty_response' });
        }
      } else if (response.status === 401 || response.status === 403) {
        Logger.info(`[Plano EnerGov] 🔒 OAuth 2.0 required (${response.status}) - Expected per docs`);
        discoveryResults.push({ endpoint: baseUrl, success: false, errorType: 'oauth_required' });
      } else if (response.status === 404) {
        Logger.info(`[Plano EnerGov] ❌ Endpoint not found (404)`);
        discoveryResults.push({ endpoint: baseUrl, success: false, errorType: 'not_found' });
      } else {
        Logger.info(`[Plano EnerGov] ⚠️  HTTP ${response.status}`);
        discoveryResults.push({ endpoint: baseUrl, success: false, errorType: `http_${response.status}` });
      }
    } catch (error: any) {
      const errorType = error.message?.includes('CORS') ? 'cors_error' : 'network_error';
      Logger.warn(`[Plano EnerGov] ❌ ${errorType}: ${error.message}`);
      discoveryResults.push({ endpoint: baseUrl, success: false, errorType });
    }
  }
...
async function trySocrataEndpoint(): Promise<Permit[]> {
  try {
    ...
    Logger.warn('[Plano Socrata] Open data portal not found');
  }
  return [];
}
